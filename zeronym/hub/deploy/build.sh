#!/bin/sh
# Reproducible build of zero-indexer-hub, using the same deterministic docker
# flags as the zeronym/shim recipe: type=oci + rewrite-timestamp +
...
# amd64 ONLY, so there is no native arm64 build of this recipe; amd64 is also the
# AWS Nitro enclave target. On an arm64 Mac this runs under Rosetta emulation,
# which is fine: the hub has no rocksdb and no libzcash_script, so a cold build
# is quick.
#
...
# left the tree clean (`git status --porcelain zebra/`), and artifacts dropped
# inside the checkout make that check noisier than it needs to be.
OUT="${OUT:-$(dirname "$ZERO_ROOT")/zero-indexer-hub-build}"

export DOCKER_BUILDKIT=1
export SOURCE_DATE_EPOCH=1

sh "$ZERO_ROOT/zeronym/hub/deploy/assemble.sh" "$CTX"
mkdir -p "$OUT"
...
docker build -f "$RECIPE" "$CTX" \
	--platform linux/amd64 \
	--target runtime \
	--output "type=oci,rewrite-timestamp=true,force-compression=true,dest=$OUT/zero-indexer-hub.tar,name=zero-indexer-hub" \
	"$@"

echo
echo "=== artifacts ==="
echo "binary:    $OUT/bin/zero-indexer-hub"
echo "OCI image: $OUT/zero-indexer-hub.tar"
echo
echo "=== binary sha256 (this is the hash an auditor reproduces) ==="
# stdin form, so the filename never enters the digest text.
sha256sum < "$OUT/bin/zero-indexer-hub" 2>/dev/null \
	|| shasum -a 256 < "$OUT/bin/zero-indexer-hub"
echo
echo "=== OCI tar sha256 ==="
sha256sum < "$OUT/zero-indexer-hub.tar" 2>/dev/null \
	|| shasum -a 256 < "$OUT/zero-indexer-hub.tar"
