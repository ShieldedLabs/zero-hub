...
# the whole basis for anyone trusting the hub with that plaintext.
#
# What this build contributes, precisely (corrected 2026-08-19): a DETERMINISTIC
# binary, so that a given commit yields a given hash on any machine. It does NOT
# contribute a value the attestation carries -- no attestation this platform
# produces contains a binary hash, and `EXPECTED_SHA256` appears nowhere in the
# Caution source. The enclave is bound to the code by `caution verify`, which
# rebuilds the EIF from the manifest's app-source repo and compares PCR0/1/2 plus
# the TLS certificate binding. Reproducibility is what makes that comparison
# mean anything; it is not itself the comparison. Without it an attestation
# proves only that SOME enclave is running, which collapses the design back into
# trusting whoever compiled it.
#
